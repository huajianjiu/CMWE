"""
GatedConv1D From
https://github.com/coosh-ed/Gated-CNN-s-Using-Keras/blob/master/GCNN.py
Small modification (add strides in arguments) to match our code

GCNN From
https://github.com/DingKe/nn_playground/blob/master/gcnn/gcnn.py
Small modification (add strides in arguments) to match our code
GCNN is not correctly configured in this keras version. It does not work.
"""
...
class GatedConv1D:
    ...
    def __init__(self, output_dim, kernel_size, strides=1, kwargs_conv={}, kwargs_gate={}):
        self.conv = Conv1D(output_dim, kernel_size, strides=strides, **kwargs_conv)
        self.conv_gate = Conv1D(output_dim, kernel_size, strides=strides, activation="sigmoid", **kwargs_gate)
